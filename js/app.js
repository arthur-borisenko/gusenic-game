...
document.getElementById("stop").addEventListener("click",gameOver)
var ww=prompt("enter max gusenic length")
...
function handleInput(dt) {
    ...
    if (true && !isGameOver) {
		...
		for (var i = 0; i < towers.length; i++) {
			if (Math.abs(player.pos[0] - towers[i].pos[0]) < 0 && 
				Math.abs(player.pos[1] - towers[i].pos[1]) < 0) {
				isClosest = true;
			}
		}
...
function reset() {
    ...
    player.pos = [canvas.width / 100, canvas.height / 10];
}
